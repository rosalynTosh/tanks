...
export class Chooser {
    ...
    private budgetDiffPrintouts: HTMLCollectionOf<HTMLSpanElement>;
    private clippyPrintouts: HTMLCollectionOf<HTMLSpanElement>;
    ...
    constructor() {
        ...
        this.budgetDiffPrintouts = document.getElementsByClassName("budgetDiff") as HTMLCollectionOf<HTMLSpanElement>;
        this.buildCostPrintouts = document.getElementsByClassName("buildCost") as HTMLCollectionOf<HTMLSpanElement>;
        this.clippyPrintouts = document.getElementsByClassName("clippy") as HTMLCollectionOf<HTMLSpanElement>;
    ...
    private updateBuildCost() {
        ...
        for (const budgetDiffPrintout of this.budgetDiffPrintouts) {
            budgetDiffPrintout.textContent = this.budget === undefined ? "--" : "$" + (this.budget - buildCost).toFixed(2);
        }
        for (const clippyPrintout of this.clippyPrintouts) {
            if (this.budget === undefined) {
                clippyPrintout.textContent = "";
            } else if (buildCost == this.budget) {
                clippyPrintout.textContent = "";
            } else if (buildCost > this.budget) {
                clippyPrintout.textContent = "Overbudget by $" + (buildCost - this.budget).toFixed(2);
                clippyPrintout.classList.add("red");
                clippyPrintout.classList.remove("green");
            } else if (buildCost < this.budget) {
                clippyPrintout.textContent = "Surplus of $" + (this.budget - buildCost).toFixed(2);
                clippyPrintout.classList.add("green");
                clippyPrintout.classList.remove("red");
            }
            this.budget === undefined ? "" : "$" + (this.budget - buildCost).toFixed(2);
        }
